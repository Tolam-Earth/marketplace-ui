...
<script lang="ts">
  ...
  export let offsets: OffsetESGSummary[];
    ...
    >
      <Head>
        <Row>
          ...
          <Cell columnId="project_type">
            <Label>Project Type</Label>
            <IconButton class="material-icons">arrow_upward</IconButton>
          </Cell>
          <Cell columnId="project_name">
            <Label>Project Name</Label>
            <IconButton class="material-icons">arrow_upward</IconButton>
          </Cell>
          <Cell columnId="project_country">
        ...
        {#if Array.isArray(slicedOffsets) && slicedOffsets.length > 0}
          {#each slicedOffsets as item, i (item.nft.token_id + item.nft.serial_number + i)}
            ...
            <Row>
              ...
              <Cell
                style="font-size: 1rem; white-space: initial; padding: 0.5rem"
                class="offsets-cell"
              >
                {item.project_type ?? '—'}
              </Cell>
              ...
              <Cell
                ...
                class="offsets-cell"
              >
                {#if item.project_country}
                  {#if item.project_region}
                    {item.project_country} ({item.project_region})
                  {:else}
                    {item.project_country}
                  {/if}
                {:else if item.project_region}
                  {item.project_region}
                {:else}
                  —
                {/if}
